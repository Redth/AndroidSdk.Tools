...
public partial class Emulator(DirectoryInfo androidSdkHome) : SdkTool(androidSdkHome)
{
	...
	{
		var locator = new EmulatorToolLocator();

		var emulator = locator.FindTool(AndroidSdkHome);
		if (emulator == null || !File.Exists(emulator.FullName))
			throw new FileNotFoundException($"Could not find {locator.ToolName}", emulator?.FullName);
			...
			while (string.IsNullOrEmpty(Serial) && !token.IsCancellationRequested)
			{
				...
				foreach (var d in devices)
				{
					try
					{
						...
						if (name?.Equals(AvdName, StringComparison.OrdinalIgnoreCase) ?? false)
						{
							Serial = d.Serial;
							break;
						}
